...
# ============================================================
#
# This script produces the tables, figures, transformed variables,
# and panel-data estimates used in the partial replication.
#
# Expected simple folder structure:
#
...
# └── outputs/
#
# The script also supports the course-style folder structure:
#
# project/
# ├── 02_original_data/
# ├── 03_clean_data/
...
# └── 08_tables/
#
# If the course-style folders exist, the script uses them.
# Otherwise, it saves all outputs inside outputs/.
# ============================================================
...
try:
    ...
    warnings.warn(
        "linearmodels is not installed. Questions 10 and 12 will be skipped. "
        "Install it with: python3 -m pip install linearmodels"
    )
...
# Simple GitHub structure: data file in the same folder as this script.
SIMPLE_DATA_FILE = SCRIPT_DIR / "Dataset_MP_Impact_functional_Distribution.xlsx"

# Course-style structure: script in a code folder, data one level above.
COURSE_BASE_DIR = SCRIPT_DIR.parent
COURSE_DATA_FILE = COURSE_BASE_DIR / "02_original_data" / "Dataset_MP_Impact_functional_Distribution.xlsx"

if SIMPLE_DATA_FILE.exists():
    BASE_DIR = SCRIPT_DIR
    DATA_FILE = SIMPLE_DATA_FILE
    OUTPUTS_DIR = BASE_DIR / "outputs"
    CLEAN_DATA_DIR = OUTPUTS_DIR / "clean_data"
    TABLES_DIR = OUTPUTS_DIR / "tables"
    FIGURES_DIR = OUTPUTS_DIR / "figures"
elif COURSE_DATA_FILE.exists():
    BASE_DIR = COURSE_BASE_DIR
    DATA_FILE = COURSE_DATA_FILE
    CLEAN_DATA_DIR = BASE_DIR / "03_clean_data"
    TABLES_DIR = BASE_DIR / "08_tables"
    FIGURES_DIR = BASE_DIR / "07_figures"
else:
    raise FileNotFoundError(
        "Could not find Dataset_MP_Impact_functional_Distribution.xlsx."
        "Place it either in the same folder as replication_main.py, or in "
        "02_original_data/ if using the course-style folder structure."
    )
...
VARIABLE_LABELS = {
    ...
    "PCOM": "Commodity price index, PCOM",
...
kept_all = sample_selection_table[
    (sample_selection_table["WR: kept in sample"]) &
    (sample_selection_table["LS: kept in sample"])
]
...
# ============================================================
# QUESTION 10. PANEL DATA ESTIMATES
# ============================================================

# The estimation sections use the clean data file produced above.
if not HAS_LINEARMODELS:
    print("Skipping Questions 10 and 12 because linearmodels is not installed.")
else:
    # ============================================================
    # Load clean data for estimation
    # ============================================================
    ...
    for dep_var in dependent_variables:
        ...
        with open(TABLES_DIR / f"q10_between_results_{dep_var}.txt", "w") as f:
        ...
        with open(TABLES_DIR / f"q10_fe_results_{dep_var}.txt", "w") as f:
        ...
        with open(TABLES_DIR / f"q10_twfe_results_{dep_var}.txt", "w") as f:
        ...
        with open(TABLES_DIR / f"q10_fd_results_{dep_var}.txt", "w") as f:
        ...
        with open(TABLES_DIR / f"q10_mundlak_results_{dep_var}.txt", "w") as f:
            f.write(str(mundlak_results.summary))

    print("\n\nAll estimations completed successfully.")

    # ============================================================
    # QUESTION 12. OWN ESTIMATION NOT DONE IN THE PAPER
    # Split sample before and after the Global Financial Crisis
    # ============================================================

    # This extension tests whether the relationship between monetary policy
    # and functional income distribution differs before and after the
    ...
    for dep_var in dependent_variables_q12:
        ...
        for period_name, (start_year, end_year) in periods_q12.items():
            ...
            # Keep only complete observations for the dependent variable and controls
            sample = sample[[dep_var] + explanatory_variables_q12].dropna()
